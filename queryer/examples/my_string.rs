...
// MyString里，String有3个word，共24字节，所以它以8字节对齐
// 所以enum的tag+padding最少8字节，整个结构占32字节。
// Mini String可以最多有30字节（再加上1字节长度和1字节tag，就是32字节）
struct MiniString {
    len: u8,
...
impl MiniString {
    // 这里new接口不暴露出去，保证传入的v的字节长度小于等于30
    fn new(v: impl AsRef<str>) -> Self {
        let bytes = v.as_ref().as_bytes();

        // 我们拷贝内容时，一定要使用字符串的字节长度
        let len = bytes.len();
