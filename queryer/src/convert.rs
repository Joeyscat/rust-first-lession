...
// 因为 Rust trait 的孤儿规则，如果想要对已有的类型实现已有的 trait，需要简单包装一下
        ...
        match sql {
            Statement::Query(q) => {
                ...
                let Select {
                    ...
                    _ => return Err(anyhow!("We only support Select Query as the moment")),
                };
            ...
            _ => Err(anyhow!("We only support Select Query as the moment")),
        }
    }
}

/// 把 SqlParser 的 Expr 转换成 DatFrame 的 Expr
...
/// 把 SqlParser 的 BinaryOperator 转换成 DatFrame 的 Operator
...
/// 把 SqlParser 的 SelectItem 转换成 DatFrame 的 Expr
...
/// 把 SqlParser 的 value 转换成 DatFrame 支持的 LiteralValue
...
mod tests {
    ...
    fn parse_sql_works() {
        ...
        let statement = &Parser::parse_sql(&JoDialect::default(), sql.as_ref()).unwrap()[0];
